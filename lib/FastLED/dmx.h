...
template <EOrder RGB_ORDER = RGB> class DMXEspController : public CPixelLEDController<RGB_ORDER> {
public:
	// initialize the LED controller
	virtual void init() { DmxEsp.init(); }

	virtual void showPixels(PixelController<RGB_ORDER> & pixels) {
		int iChannel = 1;
		while(pixels.has(1)) {
			DmxEsp.write(iChannel++, pixels.loadAndScale0());
			DmxEsp.write(iChannel++, pixels.loadAndScale1());
			DmxEsp.write(iChannel++, pixels.loadAndScale2());
			DmxEsp.update();
			pixels.advanceData();
			pixels.stepDithering();
		}
